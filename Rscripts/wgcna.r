#!/public1/home/stu_jiangqing/software/src/R-3.5.1/bin/Rscript --vanilla
#R
...
workingDir = "/public1/home/stu_jiangqing/project/my_subject/mono_genome/LA/11_WGCNA"######working dir should be changed######工作路径，可以修改，可以设置为数据存放路径
setwd(workingDir)
...
fpkm = read.table("DEGs_FPKM",header=T,comment.char = "",check.names=F)#########file name can be changed#####数据文件名，根据实际修改，如果工作路径不是实际数据路径，需要添加正确的数据路径
...
##filter
meanFPKM=0.01  ####the threshold can be changed---过滤标准，可以修改
n=nrow(datExpr0)
...
#abline(h = 30000, col = "red")##剪切高度不确定，故无红线
...
traitData = read.table("traid.txt",row.names=1,header=T,comment.char = "",check.names=F)########trait file name can be changed######性状数据文件名，根据实际修改，如果工作路径不是实际性状数据路径，需要添加正确的数据路径
dim(traitData)
names(traitData)
...
# Define numbers of genes and samples(热图)
...
##########example:royalblue and CK（挑选特定模块）
#module="royalblue"
