...
'''
需求：找到割手密基因组中含有三组或者四组等位的基因,将每组等位中的3或4个基因的CRE输出
'''
...
def Find_allele_table_with_3or4_genes_or_more(allele_file):
    ...
    allele_3ormore = pd.concat(concat_row_list)
    # allele_3ormore.to_excel(result_file, header=True, index=False)
    return allele_3ormore
...
def get_3or4_allele_gene_id(allele_file,allgenome_CRE_file,output_path):
    ...
    example(allele_file:Sspon.alleleTable20180810.xlsx,
            ...
            output_path:output)
    '''
    df = Find_allele_table_with_3or4_genes_or_more(allele_file)
    ...
    for index,row in df.iterrows():
        ...
        for i in row_list:
            if '|' in str(i):
                ...
                row_concat.append(i)
        concat_list.append(row_concat)
    result = pd.DataFrame(concat_list)
    print(result)
    ...
    for index,row in result.iterrows():
        ...
        query_df.to_csv(output_path+'/'+'row_'+str(index)+'.tab',sep='\t',header=True,index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--crematirx', required=True, help='Import the genome-wide CRE file')
