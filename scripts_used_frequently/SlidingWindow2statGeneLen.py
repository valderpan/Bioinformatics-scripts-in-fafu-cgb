        ...
        for line in lines:
            ...
            if line_list[2] == 'gene':
                if line_list[0] not in chr_tree:
                    ...
                    chr_tree[line_list[0]].addi(int(line_list[3]),int(line_list[4]),int(line_list[4])-int(line_list[3])+1)
                else:
                    chr_tree[line_list[0]].addi(int(line_list[3]), int(line_list[4]), int(line_list[4])-int(line_list[3])+1)
    return chr_tree
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        description='''Sliding window to stat gene length''',
