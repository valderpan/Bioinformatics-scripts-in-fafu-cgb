...
def Separate_groupAndSort_blocks(group_file,query_list):
    ...
    return conserverd_group_num,nonconserverd_group_concat,conserverd_group_concat

def SortOutput_conserverd_groups(group_file,conserverd_group_num,conserverd_group_concat):
    '''
    排序并输出10组保守的group
...
def split_num():
    all_groups_num = [i for i in range(1,120)]
    split_group_num = [all_groups_num[i:i+20] for i in range(0,120,20)]
    return split_group_num

def SortOutput_nonconserverd_groups(group_file,conserverd_group_num,nonconserverd_group_concat):#,conserverd_group_concat,nonconserverd_file):
...
def correct_chr(conserverd_group,nonconserverd_group,nonconserverd_sheet,tmp_output_file):
    ...
    writer = pd.ExcelWriter(tmp_output_file)
    #---------conserverd1----------------
...
def get_run_script(xlsx1,xlsx2,xlsx3,xlsx4,xlsx5,xlsx6,Output_script):
    with open(Output_script,'w') as f:
        ...
        f.write('python {} -c conserverd_group.xlsx -n nonconserverd_group_1_20.xlsx -s 0 -o 0.xlsx'.format(sys.argv[0])+'\n')
        for i in range(0,xlsx1_20_index-1):
            f.write('python {} -c {}.xlsx -n nonconserverd_group_1_20.xlsx -s {} -o {}.xlsx'.format(sys.argv[0],i,i+1,i+1)+'\n')
        ...
        for i in range(0+xlsx1_20_index-1,xlsx21_40_index+xlsx1_20_index-1):
            f.write('python {} -c {}.xlsx -n nonconserverd_group_21_40.xlsx -s {} -o {}.xlsx'.format(sys.argv[0],i,i-xlsx1_20_index+1,i+1)+'\n')
        ...
        for i in range(0 + xlsx1_20_index+xlsx21_40_index-1 , xlsx41_60_index + xlsx1_20_index +xlsx21_40_index-1):
            f.write('python {} -c {}.xlsx -n nonconserverd_group_41_60.xlsx -s {} -o {}.xlsx'.format(sys.argv[0],i, i -xlsx1_20_index-xlsx21_40_index+1,i + 1)+'\n')
        ...
        for i in range(0 + xlsx1_20_index + xlsx21_40_index + xlsx41_60_index-1, xlsx61_80_index + xlsx1_20_index + xlsx21_40_index+xlsx41_60_index-1):
            f.write('python {} -c {}.xlsx -n nonconserverd_group_61_80.xlsx -s {} -o {}.xlsx'.format(sys.argv[0],i, i -xlsx1_20_index - xlsx21_40_index - xlsx41_60_index+1, i + 1)+'\n')

        xlsx81_100 = xlrd.open_workbook(xlsx5)
        xlsx81_100_index = len(xlsx81_100.sheets())
        f.write('# -------' + 'xlsx81-100 total sheets num : {} -------'.format(str(xlsx81_100_index))+'\n')
        for i in range(0 + xlsx1_20_index + xlsx21_40_index + xlsx41_60_index + xlsx61_80_index-1,
                       xlsx81_100_index + xlsx1_20_index + xlsx21_40_index + xlsx41_60_index + xlsx61_80_index-1):
            f.write('python {} -c {}.xlsx -n nonconserverd_group_81_100.xlsx -s {} -o {}.xlsx'.format(sys.argv[0],i, i - xlsx1_20_index - xlsx21_40_index - xlsx41_60_index - xlsx61_80_index+1,i + 1)+'\n')

        xlsx101_end = xlrd.open_workbook(xlsx6)
        xlsx101_end_index = len(xlsx101_end.sheets())
        last_sheetname = xlsx101_end.sheet_names()[-1]
        f.write('# -------' + 'xlsx101-end total sheets num : {} -------'.format(str(xlsx101_end_index)) + '\n')
        for i in range(0 + xlsx1_20_index + xlsx21_40_index + xlsx41_60_index + xlsx61_80_index + xlsx81_100_index -1,
                       xlsx101_end_index + xlsx81_100_index + xlsx1_20_index + xlsx21_40_index + xlsx41_60_index + xlsx61_80_index-1):
            f.write('python {} -c {}.xlsx -n nonconserverd_group_101_end.xlsx -s {} -o {}.xlsx'.format(sys.argv[0],i,i - xlsx1_20_index - xlsx21_40_index - xlsx41_60_index - xlsx61_80_index - xlsx81_100_index +1,i + 1)+'\n')
    f.close()


def main(args):
    if args.groupfile and args.querylist:
        group_file = args.groupfile
        query_list = args.querylist
        conserverd_group_num,nonconserverd_group_concat,conserverd_group_concat = Separate_groupAndSort_blocks(group_file, query_list)
        SortOutput_nonconserverd_groups(group_file,conserverd_group_num,nonconserverd_group_concat)
        SortOutput_conserverd_groups(group_file,conserverd_group_num,conserverd_group_concat)

    if args.congroup and args.noncongroup:
        conserverd_group = args.congroup
        nonconserverd_group = args.noncongroup
        nonconserverd_sheet = args.nonsheet
        tmp_output_file = args.tmpoutput
        correct_chr(conserverd_group, nonconserverd_group, nonconserverd_sheet, tmp_output_file)
    ...
    if args.xlsx1 and args.xlsx2:
        ...
        get_run_script(xlsx1, xlsx2, xlsx3, xlsx4, xlsx5, xlsx6,Output_script)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--groupfile',  help='Input the uncorrected chromosome group file')
    parser.add_argument('-q', '--querylist', nargs='+', help='Input the id of the group to keep (eg:1,11,21,31...)')

    parser.add_argument('-c', '--congroup',  help='Input conservative block file(.xlsx)')
    parser.add_argument('-n', '--noncongroup', help='Input non-conservative block file(.xlsx)')
    parser.add_argument('-s', '--nonsheet', type=int,  help='Input non-conservative file sheetID')
    parser.add_argument('-o', '--tmpoutput', help='Output result file(.xlsx)')
    ...
    parser.add_argument('-x5', '--xlsx5', help='Input nonconserverd_group_81-100(.xlsx)')
    parser.add_argument('-x6', '--xlsx6', help='Input nonconserverd_group_101-end(.xlsx)')
